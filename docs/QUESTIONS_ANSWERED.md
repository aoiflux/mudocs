...
1. Probe execution requires runtime setting=1.
...
1. Remote scan manager integration exists behind
   runtime setting.
2. Mode gate is runtime setting=off|observe|enforce.
...
4. Trusted key source is runtime setting (with local bootstrap
   fallback if unset).

...
✅ Sandboxie (Registry, DLL injection, runtime configuration settings)
✅ Cuckoo (Cuckoo DLL, runtime configuration settings, file paths)
✅ WINE (Registry, DLLs, runtime configuration settings)

### Linux Detects
✅ Docker (/.dockerenv, cgroup markers, hostname patterns)
✅ Kubernetes (Service runtime configuration settings, certificates)
✅ LXC (Process markers, cgroup IDs, AppArmor profiles)
✅ systemd-nspawn (runtime configuration settings, cgroup analysis)
...
✅ UTM (UTM app, QEMU processes, runtime configuration settings)
✅ QEMU (QEMU binaries, CPUID detection)
✅ Colima (Docker socket, Colima directories, processes)
✅ macOS App Sandbox (runtime configuration settings, Home container markers)
